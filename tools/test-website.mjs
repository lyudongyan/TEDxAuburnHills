...
assert.match(source, /const siteRevision = "20260909-attend-suburban";/);
...
for (const name of readdirSync(root).filter(name => name.endsWith('.html'))) {
  ...
  const version = '20260909-attend-suburban';
...
assert.equal([...speakers.matchAll(/<section\b[^>]*\bspeaker-profile\b[^>]*>/g)].length, 12, 'all twelve speakers use the profile layout');
assert.equal([...speakers.matchAll(/<section\b[^>]*\bprofile-reverse\b[^>]*>/g)].length, 6, 'six profiles use the mirrored layout');
assert.match(css, /@media \(min-width: 821px\)[\s\S]*?\.speaker-profile \.profile-card\s*\{[^}]*grid-template-columns: minmax\(240px, 340px\) minmax\(0, 1fr\)/);
assert.match(css, /\.speaker-profile\.profile-reverse \.profile-card\s*\{[^}]*grid-template-columns: minmax\(0, 1fr\) minmax\(240px, 340px\)/);
assert.match(css, /\.speaker-profile \.profile-visual\s*\{[^}]*height: clamp\(400px, 35vw, 470px\);[^}]*min-height: 0;/);
assert.match(attend, /id="tickets" data-backdrop="rochester-one"/);
