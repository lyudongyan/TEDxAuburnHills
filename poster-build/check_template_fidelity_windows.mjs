...
import { DEFAULT_SLIDE_SIZE, parseArgs, requireArg } from "file:///C:/Users/Lyuwen%20Yan/.codex/plugins/cache/openai-primary-runtime/presentations/26.801.11242/skills/presentations/container_tools/artifact_tool_utils.mjs";
...
function zipNames(pptxPath) {
  const output = runCapture("tar", ["-tf", pptxPath], { encoding: "utf8" });
  if (!output) return [];
...
function readZipText(pptxPath, entryName) {
  const output = runCapture("tar", ["-xOf", pptxPath, entryName]);
